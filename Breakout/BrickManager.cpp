...
int BrickManager::checkCollision(sf::CircleShape& ball, sf::Vector2f& _direction)
{
    ...
    for (auto& brick : _bricks) {
        ...
        ParticleManager::spawnParticles(ballPosition, sf::Vector2f(_direction.x * -1, _direction.y ), NUMBER_OF_PARTICLES);
        break;
    }
    if (_bricks.size() == 0)
    {
